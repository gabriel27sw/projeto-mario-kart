...
async function logRollResult(playerName, block, diceResult, attribute) {
    console.log(`${playerName.NOME} rolou um dado de ${block} ${diceResult}`);
}
...
async function playRaceEngine(p1, p2) {
    ...
    if (block == "RETA") {
        ...
        await logRollResult(p1.NOME, block, diceResult1, p1.VELOCIDADE)
        await logRollResult(p2.NOME, block, diceResult2, p2.VELOCIDADE)
    }
    if (block == "CURVA") {
